...
class SOMMaperTest(unittest.TestCase):
    def setUp(self):
        ...
        self.kshape = (10, 10)
        self.niter = 10
        self.my_som = som.som.SOMMapper(self.kshape, self.niter)
    ...
    def testGetBMU(self):
        ...
        """
        K = np.random.rand(100, 1000).astype(np.double)
        KN = som.som.Normalizer().fit_transform(K)

        for test in np.random.randint(0, 100, size=100):
            unit, metrics = som.som.get_bmu(KN, K[test, :])
    ...
    def testPretrain(self):
        ...
        X = np.array([[0, 1, 2],
                      ...
                      [6, 7, 8],
                      [9, 10, 11]], dtype='float')
        mysom = som.som.SOMMapper(n_iter=0, kshape=(10, 20))
        mysom.fit(X)
    ...
    def testUmatrix(self):
        K = np.array([[1, 0, 0], [1, 1, 0], [1, 1, 0]]).flatten().astype(float)

        # Create a sample SOM Mapper
        mysom = som.som.SOMMapper(kshape=(3, 3))
        U_ist = som.som.build_U_matrix(K[:, np.newaxis], mysom.kshape, mysom.topology)
    ...
    def testSingleDeltas(self):
        ...
        n_nodes = 3
        for i in range(4):
            result = np.array(som.som.parallel_single_unit_deltas(X[i, :], K, infl, n_nodes, 0.1))
            control = X[i, :] - K
            # print "\nX[%d] - K\n" % i
            # print "Result\n", np.array(result)
            # print "Control\n", control
