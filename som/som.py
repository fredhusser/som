...
from _csom import parallel_single_unit_deltas
...
class SOMMapper(BaseEstimator, ClassifierMixin):
    ...
    topology: string
        ...
        square and hexagonal topologies. Only rect is implemented.
    """
    ...
    def fit(self, X, y=None, K_init=None):
        ...
        for it in np.arange(self.n_iter):
            ...
            for s in xrange(n_samples):
                ...
                unit_deltas += parallel_single_unit_deltas(X[s, :], self.K_, infl,
                                                           self.n_nodes, 1.0e-6)
    ...
    def get_bmu(self, yn):
        ...
        """
        return get_bmu(self.KN_, yn)

    def _compute_influence_kernel(self, it):
        ...
        if self.infl_decay_type == "default":
            curr_lrate = self.learning_rate * np.exp(-1.0 * it / self.iter_scale)
            infl = np.exp((-1.0 * self._dqd ** 2) / (2 * curr_max_radius ** 2))
            infl *= curr_lrate

        if self.infl_decay_type == "sigma":
            curr_lrate = ((self.learning_rate_f / self.learning_rate) ** (it / self.n_iter))
            curr_lrate *= self.learning_rate
            sigma = self.sigma * ((self.sigma_f / self.sigma) ** (it / self.n_iter))
...
def _get_node_coordinates(i_node, ncols):
...
def _get_neighbors(i_node, kshape, topology="rect"):
